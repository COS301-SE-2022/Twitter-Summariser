...
function History() {
    ...
    const getHistory = async (isMounted: boolean) => {
        try {
            const response = await axiosPrivate.post("getAllResultSet", JSON.stringify({ apiKey: localStorage.getItem("key") }), { signal: controller.signal });
                                ...
                                (history.length === 0 ? (
                                    ...
                                    history.map((data) => (
                                        <div
                                            ...
                                            key={data.id}
                                        >
                                            <HistoryCard data={data} onChange={(value: boolean) => changeShouldRender(value)} />
                                        </div>
                                    ))
                                ))}
                        </div>
