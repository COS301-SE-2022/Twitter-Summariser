...
describe("reportBlock.service", () => {
	...
	describe("getReportBlocks", () => {
		test("Get Report Blocks", async () => {
			...
			awsSdkPromiseResponse.mockReturnValueOnce(
				Promise.resolve({ Items: addedReportBlocks })
			);

			const reportBlocks = await ReportBlockService.reportBlockService.getReportBlocks(
				"1111"
			);
	...
	describe("sortReportBlocks", () => {
		test("sort report blocks", async () => {
			...
			const sorted = await ReportBlockService.reportBlockService.sortReportBlocks(
				addedReportBlocks
			);
