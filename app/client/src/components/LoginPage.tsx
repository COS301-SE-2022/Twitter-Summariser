...
function LoginPage({ loginPageData } : any) {
  ...
  const linkData = loginPageData.linkInfo;
	...
	return (
		<div
			data-testid={`page_${currPage}`}
			className={pageStyle}
		>
      <LogoComponent page="login" width = {136} height = {121} />
			<br />
      <Heading page="login" type={1} hasSecond text="Sign in to " text2="Twitter Summariser" />
			<br />
			...
			<div>
				<form onSubmit={submissionHandler} autoComplete="new-password" action="">
					<Input page="login" type="auth" inpFor="email" changeHandler={changeHandlerMethod} inputValue={changeValue} focus={focusMethod} blur={blurMethod} place="Email"/>
					{/* CODE FOR VALID EMAIL GOES HERE */}
					<br />
					<Input page="login" type="auth" inpFor="password" changeHandler={passwordHandlerMethod} inputValue={passwordValue} place="Password"/>
					<br /> <br />

          <Checkbox page="login" inpFor="remember" changeHandler={checkedChangeHandler} checked={checkedValue} text="Remember this device?" />

					{loading && (
						<ButtonComponent page="login" text="Login" type="authentication" size="large" disableId isLoading loader={loadIcon} />
					)}
					{!loading && (
            <ButtonComponent page="login" text="Login" type="authentication" size="large" disableId={!validEmail || !validPassword ? "true" : "false"} isLoading={false} />
					)}
