...
// src/functions/report/handler.ts
var handler_exports = {};
__export(handler_exports, {
  cloneReport: () => cloneReport,
  deleteReport: () => deleteReport,
  generateReport: () => generateReport,
  getAllMyDraftReports: () => getAllMyDraftReports,
  getAllMyPublishedReports: () => getAllMyPublishedReports,
  getAllPublishedReports: () => getAllPublishedReports,
  getReport: () => getReport,
  getSharedReport: () => getSharedReport,
...
// src/functions/report/handler.ts
var generateReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      const title = await services_default.resultSetServices.getResultSet(
        params.resultSetID,
        params.apiKey
      );
      const { tweets } = title;
      ...
      const report = await services_default.reportService.addReport({
        reportID: id,
...
var getAllMyDraftReports = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      const reports = await services_default.reportService.getDraftReports(params.apiKey);
      return {
        statusCode: statusCodes.Successful,
        headers: header,
...
var getAllPublishedReports = middyfy(async () => {
  try {
    const reports = await services_default.reportService.getAllPublishedReports();
    return {
      statusCode: statusCodes.Successful,
      headers: header,
      body: JSON.stringify(reports)
...
var cloneReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      const report = await services_default.reportService.getReportHelper(params.reportID);
      const oldReportId = report.reportID;
      if (!await services_default.permissionService.verifyReportRetr(
        report.status,
        params.apiKey,
        report.reportID
      ) && !await services_default.reportService.verifyOwner(oldReportId, params.apiKey)) {
        return {
      ...
      await services_default.reportService.addReport(report);
      const blocks = await services_default.reportBlockService.getReportBlocks(oldReportId);
      blocks.map(async (block) => {
        const temp = { ...block };
        temp.reportID = id;
        temp.reportBlockID = `BK-${(0, import_crypto.randomUUID)()}`;
        if (block.blockType === "RICHTEXT") {
          temp.richText = block.richText;
          let style;
          await services_default.textStyleService.getStyle(block.reportBlockID).then((value) => {
            style = value[0];
          });
          style.textStylesID = `ST-${(0, import_crypto.randomUUID)()}`;
          style.reportBlockID = temp.reportBlockID;
          await services_default.textStyleService.addStyle(style);
        } else {
          temp.tweetID = block.tweetID;
        }
        await services_default.reportBlockService.addReportBlock(temp);
        return temp;
      });
      return {
        statusCode: statusCodes.Successful,
...
var shareReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      if (await services_default.reportService.verifyOwner(params.reportID, params.apiKey)) {
        const shareTo = await services_default.creatorService.getCreator(params.email);
        if (shareTo !== void 0) {
          await services_default.permissionService.addPermission({
...
var getReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      let report = await services_default.reportService.getReportHelper(params.reportID);
      if (!await services_default.permissionService.verifyReportRetr(
        report.status,
        params.apiKey,
        report.reportID
      ...
      const per = await services_default.permissionService.getPermission(
        params.reportID,
        params.apiKey
      );
      report = await services_default.reportService.getReport(params.reportID);
      if (per !== void 0) {
        report.permission = per.type;
      } else if (services_default.reportService.verifyOwner(params.reportID, params.apiKey)) {
...
var publishReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      if (await services_default.reportService.verifyOwner(params.reportID, params.apiKey)) {
        await services_default.reportService.updateReportStatus("PUBLISHED", params.reportID);
      } else {
        return {
          statusCode: statusCodes.unauthorized,
          headers: header,
...
var unpublishReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      if (await services_default.reportService.verifyOwner(params.reportID, params.apiKey)) {
        await services_default.reportService.updateReportStatus("DRAFT", params.reportID);
      } else {
        return {
          statusCode: statusCodes.unauthorized,
          headers: header,
...
var deleteReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      if (await services_default.reportService.verifyOwner(params.reportID, params.apiKey)) {
        await services_default.reportService.updateReportStatus("DELETED", params.reportID);
      } else {
        return {
          statusCode: statusCodes.unauthorized,
          headers: header,
...
var getSharedReport = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      const re = await services_default.reportService.getSharedReports(params.apiKey);
      re.map(async (report) => {
...
var getAllMyPublishedReports = middyfy(
  async (event) => {
    try {
      const params = JSON.parse(event.body);
      const reports = await services_default.reportService.getPublishedReports(params.apiKey);
      return {
        statusCode: statusCodes.Successful,
        headers: header,
        body: JSON.stringify(reports)
