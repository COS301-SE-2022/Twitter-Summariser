...
// src/functions/reportBlock/handler.ts
var handler_exports = {};
__export(handler_exports, {
  deleteReportBlock: () => deleteReportBlock,
  editBlock: () => editBlock
});
...
var editBlock = middyfy(
  async (event) => {
    try {
      ...
      if (params.reportBlockID === void 0) {
        const id = `BK-${(0, import_crypto.randomUUID)()}`;
        text = await services_default.reportBlockService.addReportBlock({
          reportBlockID: id,
        ...
        const sid = `ST-${(0, import_crypto.randomUUID)()}`;
        styles = await services_default.textStyleService.addStyle({
          textStylesID: sid,
          reportBlockID: id,
...
var deleteReportBlock = middyfy(
  async (event) => {
    try {
      ...
      await services_default.reportBlockService.deleteReportBlock(params.reportBlockID);
      return {
        statusCode: statusCodes.Successful,
        headers: header,
